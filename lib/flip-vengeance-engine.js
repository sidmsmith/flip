...
/** Resolve one flip-four queued action without ending the flip-four sequence. */
function beginFlipFourActionResolution(state, username, card, rng) {
  if (
    card.type !== VENGEANCE_CARD_TYPES.JUST_ONE_MORE &&
    card.type !== VENGEANCE_CARD_TYPES.FLIP_FOUR &&
    !hasResolvableTargets(state, card.type, username)
  ) {
    state.discard.push(card);
    state.lastEvent = {
  ...
  switch (card.type) {
    case VENGEANCE_CARD_TYPES.JUST_ONE_MORE:
      resolveJustOneMore(state, username, rng);
      return !state.pendingChoice && !state.pendingResolution;
...
function beginFlipFourActionOnTarget(state, fromUsername, targetUsername, card, rng) {
  state.discard.push(card);
  state.lastActionFrom = fromUsername;

  switch (card.type) {
    case VENGEANCE_CARD_TYPES.JUST_ONE_MORE:
      resolveJustOneMore(state, targetUsername, rng);
      return state;
    case VENGEANCE_CARD_TYPES.FLIP_FOUR:
      return executeFlipFour(state, targetUsername, fromUsername, rng);
    case VENGEANCE_CARD_TYPES.SWAP:
      if (!hasResolvableTargets(state, card.type, targetUsername)) {
        state.lastEvent = {
          type: "action_discarded",
          from: fromUsername,
      ...
      state.pendingResolution = { type: "swap", fromUsername: targetUsername, card };
      return state;
    case VENGEANCE_CARD_TYPES.STEAL:
      if (!hasResolvableTargets(state, card.type, targetUsername)) {
        state.lastEvent = {
      ...
      state.pendingResolution = { type: "steal", fromUsername: targetUsername, card };
      return state;
    case VENGEANCE_CARD_TYPES.DISCARD:
      if (!hasResolvableTargets(state, card.type, targetUsername)) {
        state.lastEvent = {
      ...
      state.pendingResolution = { type: "discard", fromUsername: targetUsername, card };
      return state;
    default:
      return state;
  }
...
function processFlipFourPendingQueue(state, rng) {
  ...
  while (queue.length > 0) {
    const entry = queue[0];
    const keepGiveTargets = legalKeepGiveTargets(state, state.brutalMode);
    const onlySelf =
      keepGiveTargets.length === 1 && keepGiveTargets[0].username === targetUsername;

    if (entry.category === "modifier") {
      if (!onlySelf) {
        setPendingKeepGive(state, entry.card, targetUsername, "modifier");
        return state;
      }
      giveModifier(state, targetUsername, entry.card);
      queue.shift();
      continue;
    }
    ...
    const done = beginFlipFourActionResolution(state, targetUsername, entry.card, rng);
...
function executeActionOnTarget(state, fromUsername, targetUsername, card, rng) {
  ...
  switch (card.type) {
    ...
    case VENGEANCE_CARD_TYPES.SWAP:
      if (!hasResolvableTargets(state, card.type, targetUsername)) {
        state.lastEvent = { type: "action_discarded", from: fromUsername, target: targetUsername, card, reason: "no_targets" };
        return continueAfterChoice(state, rng);
      }
      state.pendingResolution = { type: "swap", fromUsername: targetUsername, card };
      return state;
...
export function playKeepGive(state, username, targetUsername, rng = Math.random) {
  ...
  const user = username.toLowerCase();
  let target = targetUsername.toLowerCase();
  ...
  if (category === "modifier") {
    ...
    if (inFlipFour) return advanceFlipFourAfterQueuedCard(state, rng);
    return continueAfterChoice(state, rng);
  }

  if (
    !hasResolvableTargets(state, card.type, target === user ? user : null) &&
    card.type !== VENGEANCE_CARD_TYPES.JUST_ONE_MORE
  ) {
  ...
  if (target === user) {
    if (inFlipFour) {
      const done = beginFlipFourActionResolution(state, user, card, rng);
      if (done) return advanceFlipFourAfterQueuedCard(state, rng);
      return state;
    }
  ...
  if (inFlipFour) {
    beginFlipFourActionOnTarget(state, user, target, card, rng);
    if (state.pendingResolution || state.pendingChoice) return state;
