...
export function scoreVengeanceTableau(player, flip7Bonus = false, brutalMode = false) {
  if (player.status === "busted") {
    if (!brutalMode) return 0;
    let sum = 0;
  ...
  if (!brutalMode) sum = Math.max(0, sum);
  return sum;
}

function projectedRoundScore(player, brutalMode) {
  let score = scoreVengeanceTableau(player, false, brutalMode);
  if (player.flip7ThisRound && player.status !== "busted" && !brutalMode) {
    score += FLIP7_BONUS;
  }
  if (!brutalMode) score = Math.max(0, score);
  return score;
}

function cardPickSortValue(card) {
  if (card.type === VENGEANCE_CARD_TYPES.MOD_DIVIDE) return -100;
  if (card.type === VENGEANCE_CARD_TYPES.MOD_MINUS) return -card.value;
  if (card.type === VENGEANCE_CARD_TYPES.NUMBER) return card.value;
  return 500;
}

function sortPickableCards(state, cards) {
  const order = new Map(state.players.map((p, i) => [p.username, i]));
  return [...cards].sort((a, b) => {
    const byPlayer = (order.get(a.username) ?? 99) - (order.get(b.username) ?? 99);
    if (byPlayer !== 0) return byPlayer;
    return cardPickSortValue(a.card) - cardPickSortValue(b.card);
  });
}
...
function canReceivePendingCard(state, player, category, card) {
  ...
  if (
    ...
    return player.status === "active" || player.status === "stayed";
  }
  return false;
}

function directResolutionType(card) {
  if (card.type === VENGEANCE_CARD_TYPES.STEAL) return "steal";
  if (card.type === VENGEANCE_CARD_TYPES.DISCARD) return "discard";
  if (card.type === VENGEANCE_CARD_TYPES.SWAP) return "swap";
  return null;
}

function isDirectCardAction(card) {
  return !!directResolutionType(card);
}

function startDirectCardAction(state, resolverUsername, card, rng, { inFlipFour = false, assigner = null } = {}) {
  const resolver = resolverUsername.toLowerCase();
  const resType = directResolutionType(card);
  if (!resType) throw new Error("Not a direct card action.");

  const afterNoOp = () =>
    inFlipFour ? advanceFlipFourAfterQueuedCard(state, rng) : continueAfterChoice(state, rng);

  if (!hasResolvableTargets(state, card.type, resolver)) {
    discardActionCard(state, card);
    state.lastEvent = {
      type: "action_discarded",
      from: assigner || resolver,
      card,
      reason: "no_targets",
  ...
  state.lastActionFrom = assigner || resolver;
  state.lastEvent = { type: "action_drawn", from: resolver, card };
  state.pendingResolution = {
    type: resType,
    fromUsername: resolver,
    assignedBy: assigner || resolver,
    card,
  };
  return state;
}

function activatePendingChoiceEntry(state, entry, rng) {
  if (entry.category === "action" && isDirectCardAction(entry.card)) {
    state.turnIndex = state.players.findIndex((p) => p.username === entry.fromUsername);
    return startDirectCardAction(state, entry.fromUsername, entry.card, rng);
  }
  state.pendingChoice = {
  ...
  state.turnIndex = state.players.findIndex((p) => p.username === entry.fromUsername);
  return tryAutoAssignPendingChoice(state, entry.fromUsername, rng);
}

function endRoundOnFlip7(state, username) {
  if (state.brutalMode) {
    state.pendingBrutalFlip7 = { username, awaitingChoice: true };
    state.phase = "brutal_choice";
    return state;
  }
  return endRound(state, "flip7");
...
function endRound(state, reason) {
  ...
  for (const player of state.players) {
    ...
    let score = scoreVengeanceTableau(player, false, state.brutalMode);
...
function resolveJustOneMore(state, targetUsername, rng) {
  ...
  if (result.modifier || result.action) {
    if (isDirectCardAction(card)) {
      ...
      return startDirectCardAction(state, targetUsername, card, rng);
    }
...
function processFlipFourPendingQueue(state, rng) {
  ...
  while (queue.length > 0) {
    ...
    if (entry.category === "action" && isDirectCardAction(entry.card)) {
      queue.shift();
      const result = startDirectCardAction(state, targetUsername, entry.card, rng, {
        inFlipFour: true,
        assigner: fromUsername,
      });
      if (state.pendingResolution || state.phase === "round_end" || state.phase === "brutal_choice") {
        return result;
      }
      continue;
    }
...
function executeFlipFour(state, targetUsername, fromUsername, rng) {
  ...
  for (let i = 0; i < 4 && !stoppedEarly; i++) {
    ...
    if (result.flip7) {
      state.lastEvent = { type: "flip_four", from: fromUsername, target: targetUsername };
      return endRoundOnFlip7(state, targetUsername);
    }
  }

  if (target.flip7ThisRound) {
    state.lastEvent = { type: "flip_four", from: fromUsername, target: targetUsername };
    return endRoundOnFlip7(state, targetUsername);
  }
...
export function dealOpeningCards(state, { isNewGame = false, rng = Math.random } = {}) {
  ...
  if (queue.length > 0) {
    const first = queue[0];
    return activatePendingChoiceEntry(state, first, rng);
  } else {
...
function continueAfterChoice(state, rng) {
  if (state.resolvingOpeningDeal && state.pendingChoiceQueue.length > 0) {
    ...
    if (state.pendingChoiceQueue.length > 0) {
      ...
      return activatePendingChoiceEntry(state, next, rng);
    }
...
export function hit(state, username, rng = Math.random) {
  ...
  if (result.modifier || result.action) {
    ...
    if (isDirectCardAction(card)) {
      return startDirectCardAction(state, user, card, rng);
    }
...
export function playKeepGive(state, username, targetUsername, rng = Math.random) {
  ...
  if (category === "action" && isDirectCardAction(card)) {
    if (state.pendingChoice.fromUsername !== user) {
      throw new Error("Only the assigning player can choose the recipient.");
    }
    state.pendingChoice = null;
    const inFlipFour = !!state.flipFourPending;
    return startDirectCardAction(state, user, card, rng, { inFlipFour, assigner: user });
  }
...
export function resolveCardAction(state, username, payload = {}, rng = Math.random) {
  ...
  if (pr.type === "swap") {
    ...
    if (p1.status === "busted" || p2.status === "busted") {
      throw new Error("Cannot swap busted players' cards.");
    }
  ...
  if (pr.type === "steal") {
    ...
    if (victim.status === "busted") throw new Error("Cannot steal from a busted player.");
  ...
  if (pr.type === "discard") {
    ...
    if (victim.status === "busted") throw new Error("Cannot discard a busted player's cards.");
...
export function publicState(state, forUsername) {
  ...
  return {
    ...
    players: state.players.map((p) => {
      const roundProj = projectedRoundScore(p, state.brutalMode);
      return {
        username: p.username,
        ...
        hasZero: hasZero(p),
        tableauScore: scoreVengeanceTableau(p, false, state.brutalMode),
        projectedRoundScore: roundProj,
        projectedTotalScore: p.totalScore + roundProj,
      };
    }),
    ...
    legalTargets:
      me && pending?.fromUsername === me
        ...
        : [],
    swappableCards:
      pr?.type === "swap" || pr?.type === "steal" || pr?.type === "discard"
        ? sortPickableCards(state, faceUpCards(state))
        : [],
  };
}
