...
export function publicState(state, forUsername) {
  ...
  return {
    ...
    players: state.players.map((p) => {
      const hidden = p.status === "busted";
      return {
        username: p.username,
        seatIndex: p.seatIndex,
        numbers: hidden ? [] : p.numbers,
        modifiers: hidden ? [] : p.modifiers,
        cards: hidden ? [] : [...p.numbers, ...p.modifiers],
        hiddenCardCount: hidden ? p.numbers.length + p.modifiers.length : 0,
        status: p.status,
        roundScore: p.roundScore,
        totalScore: p.totalScore,
