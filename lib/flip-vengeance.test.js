...
describe("flip-vengeance-engine", () => {
  ...
  it("swap that creates a duplicate busts a stayed player", () => {
    ...
    assert.equal(scoreVengeanceTableau(sidney, false, false), 0);
  });
  ...
  it("busted player with -8 before bust scores 0 in normal mode", () => {
    const player = {
      status: "busted",
    ...
    assert.equal(scoreVengeanceTableau(player, false, false), 0);
  });

  it("busted player with -8 scores -8 in brutal mode", () => {
    const player = {
      status: "busted",
      numbers: [numberCard(10, "a"), numberCard(2, "b"), numberCard(10, "c")],
      modifiers: [minusModifier(8, "m8")],
    ...
    assert.equal(scoreVengeanceTableau(player, false, true), -8);
  });

  it("stayed player with -10 cannot score below zero in normal mode", () => {
    const player = {
      status: "stayed",
      numbers: [numberCard(5, "a")],
      modifiers: [minusModifier(10, "m10")],
      flip7ThisRound: false,
    };
    assert.equal(scoreVengeanceTableau(player, false, false), 0);
  });
  ...
  it("endRound applies brutal -8 to busted player round score", () => {
    ...
    for (const p of state.players) {
      let score = scoreVengeanceTableau(p, false, true);
  ...
  it("busted players show greyed cards but are excluded from swappable list", () => {
    let state = minimalVengeanceState({
      players: [
        {
          username: "sidney",
    ...
    assert.equal(parker.numbers.length, 2);
    assert.equal(parker.modifiers.length, 1);
    assert.equal(parker.tableauScore, 0);
