...
export function scorePlayerTableau(player, flip7Bonus = false) {
  ...
  const numbers = player.cards.filter((c) => c.type === CARD_TYPES.NUMBER);
  const bonuses = player.cards.filter((c) => c.type === CARD_TYPES.BONUS);
  const multipliers = player.cards.filter((c) => c.type === CARD_TYPES.MULTIPLIER);

  let sum = numbers.reduce((s, c) => s + c.value, 0);
  sum += bonuses.reduce((s, c) => s + c.value, 0);

  for (const _ of multipliers) {
    sum *= 2;
  }

  if (flip7Bonus && player.flip7ThisRound) {
    sum += FLIP7_BONUS;
  }
